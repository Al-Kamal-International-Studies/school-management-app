 ...
 * file under a meaningful name — <student name>_<student ID>_<academic
 * year>.pdf — instead of the opaque `<uuid>.pdf` it's actually stored as
...
export async function getAdmissionPdfUrlAction(admissionId: string): Promise<{ url?: string; error?: string }> {
  ...
  const { data: admission } = await supabase
    .from("admissions")
    .select("pdf_file_path, student_full_name, enrollment_number, registration_date")
    .eq("id", admissionId)
    .single();
  ...
  const downloadName = admission.enrollment_number
    ? buildAdmissionPdfFilename({
        ...
        academicYear: academicYearLabel(new Date(admission.registration_date)),
      })
    : true;
