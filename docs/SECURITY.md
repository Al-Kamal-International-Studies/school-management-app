...
**Phase 3 — in progress.** Host approved: Vercel (Hobby plan for now, switching to Pro post-launch — the org's explicit choice, made with the Hobby ToS gray-area risk understood). GitHub Organization and Vercel account/team both created by the org. **Currently blocked on one thing:** pushing this repo to GitHub — Claude's browser-automation tools can only drive Chrome (via an installed extension), and the org's GitHub login lives in a Brave profile instead, which those tools can only view, not click/type into (a deliberate safety restriction, not a bug). Two ways to unblock, either works: (1) sign into the same GitHub account inside the Chrome tab Claude already has open, or (2) create an empty repo in Brave and hand Claude the URL — a plain `git push` then triggers a normal browser-based login prompt to approve, no browser automation needed for that part. Once resolved: push code, connect the repo in Vercel, set production env vars, first deploy, update `NEXT_PUBLIC_SITE_URL` + Supabase Auth redirect URLs to the real domain — HTTPS/HSTS then comes automatically via Vercel.

**Phase 4 — groundwork done, activation blocked on the same GitHub access issue above.** `.github/workflows/ci.yml` (typecheck/lint/build/audit on every PR), `.github/workflows/codeql.yml` (SAST), `.github/dependabot.yml` (dependency update PRs) are all written and committed locally, ready to take effect the moment the repo exists on GitHub. `docs/VULNERABILITY_RESPONSE.md` (severity levels, response times, what "fixed" means for an RLS-layer finding specifically) is done and doesn't depend on GitHub at all. `npm audit` run and clean (0 vulnerabilities, 480 dependencies) as an immediate, unblocked SCA baseline.
