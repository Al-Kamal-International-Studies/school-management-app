...
**Status:** Phases 1–4 complete (Part 3); a follow-on account-security feature pass (Part 4 below) is also complete. All ten migrations (`0016` through `0025`) are **applied to the live production database** (via the SQL Editor, with the user's explicit permission and using their own authenticated session — not a service-role bypass) and independently verified: F1's privilege-escalation exploit was actually attempted against production (simulating the exact session of a real test account, inside a rolled-back transaction) and confirmed rejected with `42501: new row violates row-level security policy`. Admin MFA was verified end-to-end with real computed TOTP codes against a disposable test admin account created and fully deleted for that purpose — no real admin account (Muhammad's or the operational one) ever had a factor enrolled by anyone but the account holder themselves. The app is now **live in production** at `https://school-management-app-nine-beryl.vercel.app` (Vercel), and the same security headers verified locally were re-confirmed against that live URL via `curl -sD -` — CSP (including `upgrade-insecure-requests`, correctly production-only), HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, and Permissions-Policy all present. `npm run typecheck && npm run lint && npm run build` all clean throughout, including after the Part 4 additions below. See HANDOVER.md Part 3 and Part 4 for the full narrative.
...
**Phase 2 — done this pass:**
- ~~Admin MFA~~ **Done.** TOTP via Supabase Auth's native MFA API — `/mfa/setup` (mandatory first-time enrollment) and `/mfa/verify` (challenge on later logins), gated inside `requireRole()` so it applies everywhere an admin lands, not just `/admin/*`. Verified end-to-end against a disposable throwaway admin account (created and fully deleted afterward, specifically so no real admin account — Muhammad's or the main operational one — ever had a factor enrolled that only Claude knew the secret to): enrollment with a real computed TOTP code, sign-out, sign-back-in correctly routing to `/mfa/verify` (not setup, since a factor now exists) with a second real code, Settings showing "Enabled," and the `mfa_enrolled` event landing in the audit log. One correction made while building this: the original plan below called for a per-device "active sessions list + revoke one" UI — checked the installed SDK's actual type definitions and there is no session-enumeration API exposed (`GoTrueAdminApi` has no `listSessions`/`revokeSession`). What *is* real and shipped instead: a "sign out of all other sessions" button (`supabase.auth.signOut({scope: "others"})`, confirmed against the SDK types), available to every role in Settings — the honest equivalent of what's actually possible, not the fabricated version.
- ~~File upload validation pass~~ **Done.** Both Storage buckets now have server-enforced `file_size_limit`/`allowed_mime_types` (migration `0020`, applied and verified live) — the real, unbypassable fix, not just the client-side/app-level checks that existed before (which a direct Storage API call could skip entirely).
- ~~Extend audit logging to teacher-side mutations~~ **Done.** Grades, attendance (once per bulk submit, not per student — a full class roster in one action would otherwise flood the log), progress entries, remarks, behaviour log, exams, assignments, and submission grading all now write an audit entry.
- Announcements/parent-audience gap — **not done, out of scope for this pass.** Flagged in HANDOVER_2 §19/§20 as a pre-existing product gap (parents only see `'all'`-audience announcements), adjacent to this security work but a feature change, not a vulnerability — left for a product conversation rather than bundled in unasked.

**Phase 3 — done.** Host: Vercel (Hobby plan, org's explicit choice — see HANDOVER.md Part 3 §12 for the trade-off this required: the GitHub repo was made **public** to satisfy Hobby's "no private-org-repo deploys" restriction, a decision made with the user's explicit sign-off, not unilaterally). GitHub repo pushed to `Al-Kamal-International-Studies/school-management-app`, Vercel project connected via GitHub App + OAuth (user-approved), all production env vars set, first deploy succeeded, `NEXT_PUBLIC_SITE_URL` and Supabase Auth → URL Configuration's Site URL + redirect URLs updated to the real domain (`https://school-management-app-nine-beryl.vercel.app`) and redeployed. HTTPS/HSTS confirmed live via Vercel automatically. Full narrative, including the OAuth-authorization checkpoint and the Hobby-vs-public-repo decision: HANDOVER.md Part 3.

**Phase 4 — active and verified.** `.github/workflows/ci.yml` (typecheck/lint/build/audit on every PR), `.github/workflows/codeql.yml` (SAST), `.github/dependabot.yml` (dependency update PRs) are committed and running for real against the live GitHub repo. Dependabot opened 5 PRs within minutes of the repo going live (`zod`, `eslint`, `typescript`, `@types/node`, `tailwindcss` version bumps) — CI correctly passed the safe minor/patch bumps and correctly failed the two major-version bumps (`typescript` 5→7, `tailwindcss` 3→4, both genuinely breaking changes needing manual review, not a CI bug). **One real gap found and fixed:** CodeQL's SARIF-upload step initially failed on every run with `Error: Resource not accessible by integration` — root cause was the `Al-Kamal-International-Studies` organization's default GitHub Actions "Workflow permissions" being set to read-only, which silently caps what any workflow's own `permissions:` block can request (the `security-events: write` already declared in `codeql.yml` was correct all along). Fixed at Settings → Actions → General for the **organization** (not the repo — the repo-level equivalent setting was greyed out/locked to "more restrictive only," confirming the org is the actual ceiling) → Workflow permissions → "Read and write permissions." Verified fixed by re-running a previously-failed CodeQL job against a live Dependabot PR: now completes in ~1 minute with 0 errors. `docs/VULNERABILITY_RESPONSE.md` (severity levels, response times, what "fixed" means for an RLS-layer finding specifically) is done. `npm audit` clean (0 vulnerabilities) as of the last check.

**Phase 5 — done this pass (HANDOVER.md Part 4).** A follow-on account-security feature pass, requested directly by Muhammad rather than found by an audit, closing several gaps this document's own §1 had flagged as open (item 2's "reset must invalidate other sessions" question, item 1's "session/device management" gap, item 9's admin-hardening MFA item):

- **Account lockout.** `failed_login_attempts` (new `profiles` column, migration `0022`) increments on every failed login; at 3, the account is deactivated via the same `setAccountBanned(true)` mechanism Phase 1's F3 fix built, plus an audit log entry. This is a new, permanent-until-admin-reactivates counter, distinct from and additional to the existing Phase 1 rate limiter (which only throttles guessing speed, doesn't lock the account).
- **Current-password verification on change-password**, closing a real gap: previously a user with a live session could change their own password without proving they knew the current one. Now re-authenticates via `signInWithPassword` server-side before allowing the change.
- **Forgot-password no longer emails a reset link — it never fully closed the loop anyway** (this app has no verified outbound email sender, HANDOVER.md Part 3 §17). Replaced with an in-app admin-notification queue (`password_reset_requests`, migration `0023`, RLS deny-by-default same as `audit_logs`/`rate_limit_events`) plus a new admin-direct "set this user's password" action, both gated behind a new mandatory `must_change_password` flag (migration `0022`) that forces a real password change on next login for any admin-set or newly-created account — the temp/reset password is single-use by design, never long-lived.
- **Device cap.** Confirmed by reading the installed `@supabase/auth-js` types directly: there is genuinely no session-enumeration or per-session-revoke-by-id API in the SDK — the same honest limitation Phase 2 hit and worked around for MFA. New table `user_devices` (migration `0024`) plus a `requireRole()`-level check on every request (not just at login) closes the same kind of gap Phase 1's F3 closed for account bans: a device removed from the ledger loses access on its next navigation, even though its still-valid Supabase token would otherwise keep working. Max 3 devices per account; a 4th login attempt is gated to a management page rather than silently rejected or silently allowed.
- **Remember me.** A login checkbox controls whether the session cookie Supabase's SSR client sets is persistent (default, ~400 days) or session-only (`Max-Age`/`Expires` stripped by a custom cookie adapter when unchecked).
- **WebAuthn biometric login.** Real Face ID/fingerprint/Windows Hello via `@simplewebauthn/server`/`@simplewebauthn/browser` (new deps, MIT-licensed, no third-party account — same "no hand-rolled cryptography" rule as every other crypto-adjacent decision in this document). New table `webauthn_credentials` (migration `0025`, no update policy — the counter bump on login goes through the service-role client only). Bridges into a real Supabase session via `admin.generateLink`/`verifyOtp` (the documented pattern for a custom auth ceremony), then runs through the same post-auth checks (active/device/must-change-password) as password login via one shared `completeLogin()` helper, so the two login paths can't drift apart on any future security check added to one but not the other.

All four new/changed migrations applied to production and verified live (HANDOVER.md Part 4 §4/§11). `npm run typecheck && npm run lint && npm run build` clean after this phase, including all six new routes it added.
